    ...
    */
    ...
    /* Process the daily forecast for the next 5 days */
      for(var i = 1; i<= 5; i++)
      {
        ...
        var headName = "head" +" "+ i;
        var cardName = "card" + " "+i;
        document.getElementById(headName).innerHTML = dayOfWeek;
        document.getElementById(cardName).innerHTML = `High: ${key.maxtemp} <br /> Low: ${key.mintemp} <br /> Sunrise: ${key.astro.sunrise}<br /> Sunset: ${key.astro.sunset}`;
      }
    /*
...
function upDateWeather(){
  ...
  var url =`https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=${la.value},${lo.value}&forecast_days=6`;
  Ajax(url);
}